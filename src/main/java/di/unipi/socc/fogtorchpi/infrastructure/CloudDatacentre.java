...
public class CloudDatacentre extends ComputationalNode {
    ...
    public CloudDatacentre clone() {
        CloudDatacentre vM = this;
        Iterator it = this.virtualMachines.entrySet().iterator();
        while (it.hasNext()) {
            Map.Entry pair = (Map.Entry)it.next();
            vM.virtualMachines.put((String)pair.getKey(),(Hardware)pair.getValue());
        }
        return vM;
    }
    ...
    @Override
    public boolean connectsToThing(String thingId) {
        return false;
    }
