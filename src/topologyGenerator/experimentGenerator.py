    ...
    with open(path, "r") as f:
        for line in f.readlines():
            ...
            if line[0].isdigit():
                ...
                tiers[node] = level
                if level >=4 and node not in idcloud:
...
#        print st
    ...
    idT = 0
    for i in G.nodes:
        if np.random.random()<= probThings and i not in idcloud:
            print i
...
#    idCloud = [153]
    ...
    G = getGraph(path,idCloud)
    nx.write_gexf(G,"network-tmp.gexf")
    

    with open("copyJAVAInfrastructure.txt","w") as f:
